...
class MainModel extends Model
{
    ...
    public function getCommissionAmountPaidToUser($type,$amount){
        ...
        if($type == "airtime" || $type == "data"){
            ...
            $purchaser_amount = round((3 / 100) * $amount,2);
            ...
            $perc_amount = round((0.1 / 100) * $amount,2);
            $perc_amount_2 = round((0.08 / 100) * $amount,2);
            $purchaser_amount = 100;
        }

        return $purchaser_amount;
    ...
    public function getTotalAmountInAllUsersWallets(){
        $total_amount = 0;
        $query = DB::table('users')->select("total_income","withdrawn")->where('id','!=',10)->get();
        if($query->count() > 0){
            foreach($query as $row){
    ...
    public function getMlmDbIdByUserId($user_id){
        $query = DB::table('mlm_db')->where('user_id',$user_id)->get("id");
        if($query->count() == 1){
            return $query[0]->id;
        }
    }
    ...
    public function getTotalWithdrawableMlmIncome($user_id){
        ...
        $total_business_earnings = $business_sponsor_earnings + $vtu_trade_income;
    ...
    public function getTotalBusinessWithdrawableEarnings($user_id){
        ...
        $query_str = "SELECT date,amount FROM mlm_earnings WHERE user_id = ".$user_id." AND (charge_type = 3 OR charge_type = 14)";
    ...
    public function providusVtuCurl($url, $use_post, $post_data=[]){
        $auth_signature = hash("sha512","R0VFS0kgTENL:61C7FFE86689DC86A97A01B9584549F4438EAE1FA041A2D5771620E0541A2514");
        $client_id = "R0VFS0kgTENL";
        $headers = [
    ...
    public function getGsubzApiKey(){
        return "ap_ad956448584718614f2355f42a858f4e";
    }
    ...
    public function upgradeMlmAccountToBusiness($mlm_db_id,$user_id,$date,$time,$reference = NULL){
        ...
        $sponsors_sponsor_income = $this->getSponsorsSponsorBonusForBusinessPackage();
        $real_sponsors_sponsor_income = $sponsors_sponsor_income;

        $sponsors_sponsor_mlmdb_id = $this->getMlmDbParamById("sponsor",$sponsor_id);
        $sponsors_sponsor_user_id = $this->getMlmDbParamById("user_id",$sponsors_sponsor_mlmdb_id);

       

        $this->creditSponsorsSponsorRegistration($user_id,$sponsors_sponsor_user_id,$sponsors_sponsor_mlmdb_id,$real_sponsors_sponsor_income,3,$date,$time);
    ...
    public function registerUserInMlm3($user_id,$sponsor_id,$date,$time,$package,$type,$reference = NULL){
        if($this->fixUserInNextAvailableSpaceForMlm($package,1,$sponsor_id,$user_id,$date,$time,$reference)){
            if($package == 1){
                ...
                $sponsors_sponsor_user_id = $this->getMlmDbParamById("sponsor",$sponsor_id);
                $sponsors_sponsor_mlmdb_id = $this->getUsersFirstMlmDbId($sponsors_sponsor_user_id);
    ...
    public function getSponsorsSponsorBonusForBusinessPackage(){
        
        $query = DB::table('mlm_charges')->where('id', 20)->get();
        if($query->count() == 1){
            return $query[0]->amount;
        }
    }

    public function creditSponsorsSponsorRegistration($user_id,$sponsors_sponsor_user_id,$sponsors_sponsor_mlmdb_id,$real_sponsors_sponsor_income,$charge_type,$date,$time){

        if(DB::table('mlm_earnings')->insert(array('user_id' => $sponsors_sponsor_user_id,'mlm_db_id' => $sponsors_sponsor_mlmdb_id,'charge_type' => $charge_type,'amount' => $real_sponsors_sponsor_income,'vat' => 0,'date' => $date,'time' => $time))){
           

            
            $form_array = array();

            $this->updateUsersBusinessIncomeForTheMonth($sponsors_sponsor_user_id,$real_sponsors_sponsor_income);

            $total_business_income = $this->getUserParamById("total_business_income",$sponsors_sponsor_user_id);
            $new_total_business_income = $total_business_income + $real_sponsors_sponsor_income;
            ...
            if($this->updateUserTable($form_array,$sponsors_sponsor_user_id)){
                return true;
            }
            ...
            if(count($recepient_arr) > 0){
                ...
                if($_SERVER['SERVER_NAME'] !== "127.0.0.1"){
                    ...
                    if(is_array($recepient_arr)){
                        ...
                        // $mail->SMTPAuth = true;                               // Enable SMTP authentication
                        // $mail->Username = 'info@geeklickservices.com';                 // SMTP username
                        // $mail->Password = 'wp1s96VhMV8}';                           // SMTP password
                        // $mail->SMTPSecure = 'pop3';                            // Enable TLS encryption, `ssl` also accepted
                        // $mail->Port = 25;                                    // TCP port to connect to
                        
                        // $mail->setFrom('info@geeklickservices.com', 'Geeklick Services');
    ...
    public function sendOtpEmail($email,$otp){
        if(is_array($email)){
            $subject = "OTP For Geeklick Services";
            $message = "Your Otp Is: ".$otp.". Thanks For Using Geeklick Services.";
    ...
    public function buyPowerVtuCurl($url, $use_post, $post_data=[]){
        $headers = [
            'Authorization' => 'Bearer 9415749251788f582d763c30400e0a9698aeac5f909ce699c94eb56457a5a062',
            'Content-Type' => 'application/json'
        ];
        if(!$use_post){
            $response = Http::withOptions([
    ...
    public function payscribeVtuCurl($url, $use_post, $post_data=[]){
        $headers = [
            'Authorization' => 'Bearer ps_live_60f30e645fc4de31ba808f44a36a2cbc88ea7afa8cbef6a1e8ac7e8a1867ebf1',
            'Content-Type' => 'application/json',
            'Cache-Control' => 'no-cache'
        ];
        if(!$use_post){
    ...
    public function addTransactionStatus($form_array,$payscribe_data = false){  
        ...
        if(DB::table('vtu_transactions')->insert($form_array)){
            ...
            $type = $form_array['type'];
            $sub_type = $form_array['sub_type'];
            $sub_type = strtolower($form_array['sub_type']);
            if($type == "airtime" || $type == "data"){
                if($type == "data"){
                    $amount = $amount - 2;
                }
                ...
                $perc_amount_2 = round((0.08 / 100) * $amount,2);
                if($sub_type == "mtn"){
                    $purchaser_amount = round((2.5 / 100) * $amount,2);    
                }else{
                    $purchaser_amount = round((3 / 100) * $amount,2);    
                }
            ...
            if(DB::table('mlm_earnings')->insert(array('user_id' => $user_id,'mlm_db_id' => $mlm_db_id,'charge_type' => $charge_type,'amount' => $purchaser_amount,'vat' => 0,'date' => $date,'time' => $time))){
                ...
                $this->updateUsersBusinessIncomeForTheMonth($user_id,$purchaser_amount);

                $total_business_income = $this->getUserParamById("total_business_income",$user_id);
                $new_total_business_income = $total_business_income + $purchaser_amount;
