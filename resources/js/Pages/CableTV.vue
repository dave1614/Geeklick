      ...
      <div class="">
        <div class="container-fluid">
          <div class="text-right">
            ...
            <a href="https://wa.me/2348060505093?text=I Have A Problem With My Cable Tv Recharge&" class="btn btn-success" target="_blank" ><i style="font-size:17px;" class="fab fa-whatsapp"></i></i>&nbsp;&nbsp;&nbsp;&nbsp;Support</a>
          </div>
      ...
      if(Object.keys(selected_plan).length > 0){
        ...
        onSuccess: (page) => {
          ...
          if(response.success && response.order_id !== ""){
            ...
            var date = new Date().toLocaleDateString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric",hour: "numeric",minute: "numeric",second: "numeric"});
            if(!transaction_pending){
              swal({
                title: 'Info',
                text: "You Have Successfully Recharged Decoder With Number: <em class='text-primary'>" + decoder_number + ".</em> The Order Id For This Transaction Is <em class='text-primary'>" +order_id + "</em>. Do You Want To View Receipt?",
                type: 'info',
                ...
                if(self.buy_cable_request.operator == 'dstv' || self.buy_cable_request.operator == 'gotv' || (self.buy_cable_request.operator == 'startimes' && self.buy_cable_request.platform == 'club')){
                  var arr = [
                    {
                    ...
                    {
                      'key' : 'Transaction Staus',
                      'value' : 'SUCCESS',
                    },
                    {
                      'key' : 'Cable Operator',
                      'value' : self.buy_cable_request.operator,
                    },
                    {
                      'key' : 'Amount',
                      'value' :  '₦' + self.addCommas(selected_plan.amount),
                    },
                    {
                      'key' : 'Decoder Number',
                      'value' : self.buy_cable_request.decoder_number,
                    },
                    {
                      'key' : 'Customer Name',
                      'value' : self.buy_cable_request.customer_name,
                    },
                    {
                      'key' : 'BOUQUET',
                  ...
                  var arr = [
                    ...
                    {
                      'key' : 'Package Name',
                      'value' : selected_plan.package_name,
                    },
                    {
                      'key' : 'Cycle',
                      'value' : selected_plan.cycle,
                    },
                    {
                      'key' : 'Trans Id',
                      'value' : order_id,
                ...
                self.$inertia.visit(self.route('receipt_page') + '?details='+details);
              },function(dismiss){
                if(dismiss == 'cancel'){
